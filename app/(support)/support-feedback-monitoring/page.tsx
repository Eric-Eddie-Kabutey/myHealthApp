...
function SupportFeedbackMonitoring() {
    return <PageWrapper content={<>
        ...
        <section className="border-b flex md:flex-row flex-col items-start  md:items-end justify-between gap-2">
            <div className="flex flex-col">
                <p className='text-base'>Feedback Monitoring</p>
                <p className="text-sm text-gray-600">Monitor ratings from patients post-consultation or delivery.</p>
            </div>
            <div className="flex gap-3">
                <aside className="rounded w-[300px] px-2 h-9 border flex items-center gap-2">
        ...
        <section className="w-full flex flex-col gap-5">
            {
                [1, 2, 3, 4].map((mon, idx) => {
                    return <div key={`monitoring-${idx}`} className="border rounded-lg p-4 w-full flex justify-between">
                        <section className="flex flex-col gap-2">
                            <span className="flex items-center gap-2">
                                <p>John Smith</p>
                                <p className="px-3 py-1 bg-gray-100 rounded">Consultation</p>
                                <p className="text-gray-700">2025-05-30</p>
                            </span>
                            <span className="flex gap-2 items-center">
                                {[1, 2, 3, 4, 5].map((star, idx) => <Star key={`star-rating-${idx}`} className='size-4' />)}
                                2/5
                            </span>
                            <p className="text-gray-500">Long wait time, but doctor was helpful</p>
                        </section>
                        <section className="flex flex-col items-end gap-2">
                            <div className="px-3 py-1 rounded bg-red-50 text-red-700">Follow-up: Pending</div>
                            <span className="flex gap-3">
                                <Button variant={'outline'} className='rounded border-primary text-primary h-8'>Send Email</Button>
                                <Button variant={'outline'} className='rounded border-primary text-primary h-8'>Call Patient</Button>
                            </span>
                        </section>
                    </div>
                })
...
export default SupportFeedbackMonitoring