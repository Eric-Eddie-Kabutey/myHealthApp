...
function ConfirmSuspend({ open, close }: { open: any, close: () => void }) {
    ...
    return <Dialog open={!!open} onOpenChange={close}>
        <DialogContent className="main-content sm:w-[400px] flex flex-col items-center gap-3">
            <DialogTitle />
            {
                suspended ? <div className='w-full flex flex-col items-center py-5'>
                    ...
                    <p className=" mb-4 text-gray-700 w-[240px] text-center mt-3">You have successfully suspended this account.</p>
                </div> : <div className='w-full flex flex-col items-center py-5'>
                    <p className="text-lg">Are you Sure?</p>
                    <p className="text-sm mb-4 text-gray-700">You are about to suspend this account</p>
                    <br />
                    <div className="w-full grid grid-cols-2 gap-3">
                        <Button onClick={()=>close()} variant={'outline'}>Cancel</Button>
                        <Button onClick={()=>setSuspended(true)}>Suspend Account</Button>
                    </div>
                </div>
            }
