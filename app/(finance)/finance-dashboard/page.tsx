...
function FinanceDashboard() {
    ...
    return <PageWrapper content={<>
                    ...
                    return <div key={`stat-${idx}`} className="rounded-xl min-w-[270px] h-[105px] border p-4 flex gap-2">
                        ...
                        <aside className="flex flex-col">
                            ...
                            <span className={`flex items-center text-xs mt-2 ${stat.state === 'increase' ? 'text-emerald-600' : 'text-red-500'}`}>
                                {stat.state === 'increase' ? <ArrowUp className='size-4' /> : <ArrowDown className='size-4' />}
                                {stat.stateChange}
                            </span>
                        </aside>
                    </div>
        ...
        <section className="w-full h-[400px] flex gap-5">
            <div className="w-full border p-4 pt-0 flex flex-col gap-3 rounded-xl">
                <aside className="w-full min-h-14 h-14 py-3 flex items-center justify-between border-b bg-white sticky top-0">
                    <p>Revenue Overview</p>
                    <span className="flex items-center gap-2">
                        <div className='size-3 rounded-full bg-emerald-700' /> <small className='leading-0'>Revenue</small>
                        <div className='size-3 rounded-full bg-emerald-400' /> <small className='leading-0'>Revenue</small>
                    </span>
                    <div className="border flex text-sm">
                        <span className={`px-4 py-2 flex items-center`}>Daily</span>
                        <span className={`px-4 py-2 flex items-center border-x`}>Weekly</span>
                        <span className={`px-4 py-2 flex items-center border-x`}>Monthly</span>
                        <span className={`px-4 py-2 flex items-center`}>Yearly</span>
                    </div>
                </aside>
                {/* <GooglePieChart/> */}
                <GoogleCharts type='Bar' />
            </div>
            <div className="min-w-[450px] border p-4 pt-0 flex flex-col gap-3 rounded-xl overflow-y-auto">
                <aside className="w-full min-h-14 h-14 py-3 flex items-center justify-between border-b bg-white sticky top-0">
                    <p>Recent Transactions</p>
                </aside>
                {
                    [1, 2, 3, 4, 5, 6].map((rec, idx) => {
                        return <div key={`rec-${idx}`} className="w-full border-b py-3 flex items-center justify-between">
                            <aside className="flex flex-col">
                                <small>Event Registration</small>
                                <small className="text-gray-500">Today. 10:45 AM</small>
                            </aside>
                            <aside className="flex flex-col">
                                <small className='text-emerald-600'>+$150.00</small>
                                <small className="text-gray-500">Today. 10:45 AM</small>
                            </aside>
                        </div>
                    })
        ...
        <section className="w-full border p-4 pt-0 flex flex-col gap-3 rounded-xl h-[400px] overflow-y-auto">
            <aside className="w-full min-h-14 h-14 py-3 flex items-center justify-between border-b bg-white sticky top-0">
            ...
            {
                [1, 2, 3, 4, 5].map((ins, idx) => {
                    return <div key={`ins-${idx}`} className="w-full rounded-lg p-4 flex flex-col bg-gray-100">
                        <small className="text-gray-600">Daily Revenue</small>
